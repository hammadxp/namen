...
export function HomeSearch({ categories }: { categories: Category[] }) {
  ...
  return (
    <section
      className="relative grid min-h-[540px] place-content-center overflow-hidden border-b-2 border-ink bg-ink px-[clamp(1.125rem,8vw,8.125rem)] pt-[88px] pb-[100px] text-white max-[780px]:min-h-[500px] max-[780px]:pt-[68px] max-[560px]:min-h-[550px] max-[560px]:px-4"
    ...
    >
      ...
      <form
        className="relative z-[1] mt-12 grid w-full max-w-[1050px] animate-[search-in_560ms_120ms_cubic-bezier(0.2,0.72,0.2,1)_both] grid-cols-[auto_minmax(180px,1fr)_auto_auto] items-center border-2 border-ink bg-white text-ink shadow-[10px_10px_0_var(--orange)] max-[780px]:relative max-[780px]:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] max-[560px]:mt-[34px] max-[560px]:shadow-[6px_6px_0_var(--orange)]"
        onSubmit={search}
      >
        <Search
      ...
      {matches.length > 0 && (
        <div className="relative z-[1] mt-5 flex w-full max-w-[1050px] flex-wrap gap-2" aria-label="Quick matches">
          {matches.map((match) => (
            <button
