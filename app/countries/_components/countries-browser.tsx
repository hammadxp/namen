  ...
  return (
    <main className="page-shell">
      <section className="page-heading country-heading">
        ...
        <p>Browse every country in the local city index, then open its complete city list.</p>
      </section>
      <section className={cn("browser-toolbar", toolbarHidden && "is-hidden")}>
        ...
        <label className="select-field">
          <span>Sort</span>
          <select value={sort} onChange={(event) => setSort(event.target.value as CountrySort)}>
      ...
      <section className="country-grid">
        {visible.map((country) => (
          <Link className="country-card" href={`/categories/cities?country=${country.code}`} key={country.code}>
      ...
      {visible.length === 0 && (
        <div className="empty-state">
          ...
          <p>Try another name, region, or code.</p>
        </div>
      )}
    </main>
  );
